...
void sjf_scheduler()
{
    ...
    while (job_queue->size > 0)
    {
        ...
        if (p->arrival_time < cpu_counter)
        {
            ...
            enqueue_orderby_burst_time(ready_queue, p); // Enqueue based on burst time
        }
        else
        {
            break;
    ...
    // Check if any process has exited I/O
    current = waiting_queue->head;
    while (current != NULL)
    {
        p = current->data;
...
void priority_scheduler()
{
    ...
    while (job_queue->size > 0)
    {
        ...
        if (p->arrival_time < cpu_counter)
        {
            p->flag = PROCESS_FLAG_READY;
            dequeue(job_queue, p->pid);
            enqueue_orderby_priority(ready_queue, p); // Enqueue based on priority
        }
        else
        {
            break;
